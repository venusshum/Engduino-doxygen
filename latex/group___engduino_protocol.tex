...
On success R\+E\+S\+\_\+\+O\+K, otherwise error code represents particular error.
\end{DoxyReturn}
This function will set pins value. Parameters needs to be specified in K\+E\+Y-\/\+V\+A\+L\+U\+E-\/\+V\+A\+L\+U\+E block. The K\+E\+Y is a pin number. The first V\+A\+L\+U\+E donates to a pin value and the second V\+A\+L\+U\+E donates to a pulse width in milliseconds. Multiple blocks can be included in the same package. In the case of passing only one block, only K\+E\+Y-\/\+V\+A\+L\+U\+E block is allowed, representing pin number and pin value without pulse width. Be carful that a pin is already initialized as an output!
...
Pulse width\+: \begin{TabularC}{2}
\hline
\rowcolor{lightgray}{\bf Value}&{\bf Type }\\\cline{1-2}
\mbox{[}$<$ 1\mbox{]}&No pulse \\\cline{1-2}
\mbox{[}$>$ 1\mbox{]}&Pulse duration in milliseconds\\\cline{1-2}
\end{TabularC}


Input value examples\+: (Special case -\/ dig\+Analog = 0) ~\newline
 \mbox{[}4,1\mbox{]} -\/$>$ Set D4 on H\+I\+G\+H state.

Input value examples\+: (dig\+Analog = 0) ~\newline
 \mbox{[}4,0,0, 6,1,2500, 8,0,560, ...\mbox{]} -\/$>$ Set D4 on H\+I\+G\+H state, set D6 on H\+I\+G\+H state for 2500 milliseconds and set D8 on L\+O\+W state for 560 milliseconds. After the pulse duration expire the state of the pin will be inverted.

Input value examples\+: (dig\+Analog = 1) ~\newline
 \mbox{[}5,96,0 13,239,1000 ...\mbox{]} -\/$>$ Set D5\# and D13\# as P\+W\+M outputs. The second value in a block represents the duty cycle\+: between 0 (always off) and 255 (always on). The third value represents active duration. After duration expire the P\+W\+M output will be set to zero. \hypertarget{group___engduino_protocol_gad15fd39a7f4787cf6c8bbfc101df4387}{}\index{Driver for Engduino Protocol@{Driver for Engduino Protocol}!````~Engduino\+Protocol\+Class@{$\sim$\+Engduino\+Protocol\+Class}}
